 ...
 */ function offscreen() {
  let { DOMParser: DOMParserFallback } =
...
class Property {
  ...
  getDefinition() {
    ...
    const name =
      (match === null || match === void 0 ? void 0 : match[1]) || asString;
  ...
  getFillStyleDefinition(element, opacity) {
    ...
    // gradient
    if (
      typeof def.createGradient === "function" &&
      "getBoundingBox" in element
    ) {
...
class Screen {
  ...
  constructor(
    ctx,
    { fetch: fetch1 = defaultFetch$1, window: window1 = defaultWindow } = {}
  ) {
    this.ctx = ctx;
...
class Parser {
  ...
  constructor({
    fetch = defaultFetch,
    DOMParser: DOMParser1 = DefaultDOMParser,
  } = {}) {
    if (!fetch) {
      throw new Error(
        `Can't find 'fetch' in 'globalThis', please provide it via options`
    ...
    if (!DOMParser1) {
      throw new Error(
        `Can't find 'DOMParser' in 'globalThis', please provide it via options`
      );
    }
    this.fetch = fetch;
...
class Element {
  ...
  matchesSelector(selector) {
    ...
    const styleClasses =
      (_node_getAttribute = node.getAttribute) === null ||
      _node_getAttribute === void 0
        ? void 0
        : _node_getAttribute.call(node, "class");
  ...
  isFirstChild() {
    ...
    return (
      ((_this_parent = this.parent) === null || _this_parent === void 0
        ? void 0
        : _this_parent.children.indexOf(this)) === 0
    );
  }
}
...
class BoundingBox {
  ...
  addQuadraticCurve(p0x, p0y, p1x, p1y, p2x, p2y) {
    const cp1x =
      p0x +
      (2 / 3) *
        (p1x - p0x); // CP1 = QP0 + 2/3 *(QP1-QP0)
    const cp1y =
      p0y +
      (2 / 3) *
        (p1y - p0y); // CP1 = QP0 + 2/3 *(QP1-QP0)
    const cp2x =
      cp1x +
      (1 / 3) *
        (p2x - p0x); // CP2 = CP1 + 1/3 *(QP2-QP0)
    const cp2y =
      cp1y +
      (1 / 3) *
        (p2y - p0y); // CP2 = CP1 + 1/3 *(QP2-QP0)
  ...
  setContext(ctx) {
    ...
    if (!fromMeasure) {
      ...
      if (fillStyleProp.isUrlDefinition()) {
        const fillStyle = fillStyleProp.getFillStyleDefinition(
          this,
      ...
      if (strokeStyleProp.isUrlDefinition()) {
        const strokeStyle = strokeStyleProp.getFillStyleDefinition(
          this,
          strokeOpacityProp
        );
        if (strokeStyle) {
        ...
        if (strokeStyle !== "inherit") {
          ctx.strokeStyle =
            strokeStyle === "none" ? "rgba(0,0,0,0)" : strokeStyle;
        }
      }
      if (strokeOpacityProp.hasValue()) {
        const strokeStyle = new Property(
          this.document,
          "stroke",
          ctx.strokeStyle
        )
...
class TextElement extends RenderedElement {
  ...
  getGlyph(font, text, i) {
    ...
    if (font.isArabic) {
      ...
      glyph =
        ((_font_arabicGlyphs_char = font.arabicGlyphs[char]) === null ||
        _font_arabicGlyphs_char === void 0
          ? void 0
          : _font_arabicGlyphs_char[arabicForm]) || font.glyphs[char];
    } else {
  ...
  adjustChildCoordinates(ctx, textParent, parent, i) {
    ...
    const isRTL = Boolean(
      customFont === null || customFont === void 0 ? void 0 : customFont.isRTL
    );
    if (i === 0) {
  ...
  getChildBoundingBox(ctx, textParent, parent, i) {
    ...
    if (boundingBox) {
      child.children.forEach((_, i) => {
        const childBoundingBox = textParent.getChildBoundingBox(
          ctx,
          textParent,
          child,
          i
   ...
   */ getInheritedAttribute(name) {
    ...
    let current = this;
    while (
      current instanceof TextElement &&
...
class SVGElement extends RenderedElement {
  ...
  setContext(ctx) {
    ...
    if (
      ...
      (!this.parent ||
        ((_this_node_parentNode = this.node.parentNode) === null ||
        _this_node_parentNode === void 0
          ? void 0
          : _this_node_parentNode.nodeName) === "foreignObject") &&
   ...
   */ resize(width) {
    ...
    if (preserveAspectRatio) {
      if (typeof preserveAspectRatio === "string") {
        ...
        const preserveAspectRatioAttr = this.getAttribute(
          "preserveAspectRatio"
        );
...
class GradientElement extends Element {
  ...
  createGradient(ctx, element, parentOpacityProp) {
    ...
    if (this.getAttribute("gradientTransform").hasValue()) {
      ...
      const patternCanvas = document.createCanvas(
        rootView.width,
        rootView.height
      );
      const patternCtx = patternCanvas.getContext("2d");
...
class TextPathElement extends TextElement {
  ...
  findSegmentToFitChar(
    ...
    const p1 = this.getEquidistantPointOnPath(
      offset + glyphWidth,
      splineStep,
      0
    );
  ...
  pathA(pathParser) {
    let { rX, rY, sweepFlag, xAxisRotation, centp, a1, ad } =
      PathElement.pathA(pathParser);
    if (sweepFlag === 0 && ad > 0) {
      ad -= 2 * Math.PI;
    }
...
class MaskElement extends Element {
  static ignoreStyles = ["mask", "transform", "clip-path"];
  type = "mask";
  apply(ctx, element) {
    const { document } = this;
    // render as temp svg
...
class ClipPathElement extends Element {
  ...
  apply(ctx) {
    ...
    this.children.forEach((child) => {
      ...
      let transform =
        "elementTransform" in child ? child.elementTransform() : null; // handle <use />
      if (!transform) {
...
class FilterElement extends Element {
  static ignoreStyles = ["filter", "transform", "clip-path"];
  type = "filter";
  apply(ctx, element) {
    // render as temp svg
    const { document, children } = this;
    const boundingBox =
      "getBoundingBox" in element ? element.getBoundingBox(ctx) : null;
    if (!boundingBox) {
      return;
    }
    let px = 0;
    ...
    const ignoredStyles = this.removeStyles(
      element,
